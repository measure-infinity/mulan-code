...
class UNetMidBlock2D(nn.Module):
    ...
    def forward(self, hidden_states: torch.FloatTensor, temb: Optional[torch.FloatTensor] = None) -> torch.FloatTensor:
        ...
        for attn, resnet in zip(self.attentions, self.resnets[1:]):
            if attn is not None:
                hidden_states, _, _ = attn(hidden_states, temb=temb)
    ...
    def forward(
        ...
        for attn, resnet in zip(self.attentions, self.resnets[1:]):
            if self.training and self.gradient_checkpointing:
                ...
                # mid_attn.append(hidden_output[1])
                # vis_attn.append(hidden_output[2])
        ...
        for i, (resnet, attn) in enumerate(blocks):
            if self.training and self.gradient_checkpointing:
                ...
                hidden_output = attn(
                    ...
                    attention_mask=attention_mask,
                    encoder_attention_mask=encoder_attention_mask,
                    return_attns=down_attn,
                    vis_attn=vis_attn,
                    return_dict=False,
                )
                hidden_states = hidden_output[0]
                # down_attn.append(hidden_output[1])
                # vis_attn.append(hidden_output[2])
        ...
        for resnet, attn in zip(self.resnets, self.attentions):
            ...
            if self.training and self.gradient_checkpointing:
                ...
                hidden_output = attn(
                    ...
                    attention_mask=attention_mask,
                    encoder_attention_mask=encoder_attention_mask,
                    return_attns=up_attn,
                    vis_attn=vis_attn,
                    return_dict=False,
                )
                hidden_states = hidden_output[0]
                # up_attn.append(hidden_output[1])
                # vis_attn.append(hidden_output[2])
