...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        ### Replace Attn2 for convenient attention map computation.
        ...
        if processor is None:
            ...
            processor = (
                AttnProcessor() if hasattr(F, "scaled_dot_product_attention") and self.scale_qk else AttnProcessor()
            )
...
class AttnProcessor:
    ...
    def __call__(
        ...
        if is_cross_attention:
            attn_save = attention_probs.reshape(hidden_states.shape[0], attn.heads, attention_probs.shape[-2], attention_probs.shape[-1])
            attn_save = attn_save.mean(dim=1)
            if hidden_states.shape[0] > 1:
                attn_save = attn_save[1]
            return_attn = attention_probs
        else:
            attn_save = None
            return_attn = None
        ...
        return hidden_states, return_attn, attn_save
